...
// Same key used in TasksScreen — keeps them in sync
...
function QuickTaskModal({ visible, onClose }) {
  ...
  const closeModal = (cb) => {
    ...
    Animated.timing(slideAnim, { toValue: -700, duration: 250, useNativeDriver: true })
      .start(() => {
        setTaskName(''); setTaskDesc(''); setEventDate('');
        setImages([]); setActiveTab('task');
        onClose(); cb && cb();
      });
  };
  ...
  const handleSave = async () => {
    if (!taskName.trim()) {
      Alert.alert('Required', activeTab === 'task' ? 'Enter a task name.' : 'Enter an event name.');
      return;
    }
    const newEntry = {
    ...
    try {
      ...
      closeModal(() =>
        Alert.alert(
          activeTab === 'task' ? '✅ Task Saved!' : '📅 Event Saved!',
          ...
          >
            ...
            <View style={styles.toggle}>
              {['task', 'event'].map(t => (
                <TouchableOpacity
                  key={t}
                  style={[styles.toggleBtn, activeTab === t && styles.toggleBtnOn]}
                  onPress={() => setActiveTab(t)}
                >
                  <Text style={[styles.toggleText, activeTab === t && styles.toggleTextOn]}>
                    {t === 'task' ? '📋  Task' : '📅  Event'}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            {activeTab === 'event' && (
              <>
                ...
                <TextInput
                  style={styles.input}
                  placeholder="e.g. April 20, 2026 at 3:00 PM"
                  placeholderTextColor="#AAAABC"
            ...
            <View style={styles.infoBox}>
              <Text style={styles.infoText}>
                💾  Saves to your device → appears in the Tasks tab instantly.{'\n'}
    ...
    <>
      <Tab.Navigator
        screenOptions={({ route }) => ({
          ...
          tabBarIcon: ({ color }) => {
            const icons = { Dashboard: '⊞', Projects: '🗂️', Tasks: '📋', Docs: '📄' };
      ...
      >
        ...
        {/* Center FAB — opens Quick Add modal */}
        <Tab.Screen
          name="Quick"
          ...
          options={{
            ...
            tabBarIcon:   () => null,
            tabBarButton: () => (
              <QuickTaskButton onPress={() => setModalVisible(true)} />
            ),
          }}
        />

        {/* Tasks tab — shows everything saved by Quick Add */}
...
const styles = StyleSheet.create({
  tabBar: {
    ...
    borderTopColor: '#EBEBF0', borderTopWidth: 1,
  ...
  fabIcon: { color: '#4ECDC4', fontSize: 28, lineHeight: 32 },
  ...
  handle: {
    width: 40, height: 4, backgroundColor: '#DEDEE8', borderRadius: 2,
  ...
  input: {
    ...
    fontSize: 14, color: '#1A1A2E', marginBottom: 14,
  },
  ...
  attachBtn: {
    ...
    borderRadius: 12, borderWidth: 1.5, borderColor: '#EBEBF0',
