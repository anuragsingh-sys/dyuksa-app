...
// =============================================================================
// Pure rendering file — zero fetch() calls, zero business logic.
// All data and logic comes from useDashboard().
// =============================================================================
...
function SectionHeader({ children, right, style, isDark }) {
  return (
    <View style={[{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }, style]}>
      <Text style={{ fontSize: 13, fontWeight: '700', color: isDark ? '#9AA3B2' : T.ink2, letterSpacing: 0.3 }}>
        {children}
      </Text>
      {right}
    </View>
...
export default function DashboardScreen() {
  ...
  const OVERVIEW = [
    {
      label: 'Total Projects', value: projects.length, color: T.cBlue, soft: T.cBlueSoft, icon: 'folder',
      pct: `${projects.length} total`,
      spark: [2, 3, 4, 5, 6, 7, 8, projects.length || 1],
      onPress: () => { try { navigation.jumpTo('Projects'); } catch { navigation.navigate('Projects'); } },
    },
    {
      label: 'Total Documents', value: totalDocs, color: T.cGreen, soft: T.cGreenSoft, icon: 'file-text',
      pct: `${totalDocs} total`,
      spark: [10, 15, 20, 30, 35, 40, 50, totalDocs || 1],
      onPress: () => navigation.navigate('Docs'),
    },
    {
      label: 'Total Tasks', value: totalTasks, color: T.cYellow, soft: T.cYellowSoft, icon: 'check-square',
      pct: `${pendingTasks} pending`,
      spark: sparkFor(() => true),
      onPress: () => { try { navigation.jumpTo('Tasks'); } catch { navigation.navigate('Tasks'); } },
    },
    {
      label: 'Completed', value: completedTasks, color: T.cPurple, soft: T.cPurpleSoft, icon: 'check-circle',
      pct: `${totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0}% done`,
      spark: sparkFor(t => ['completed', 'deployed'].includes(t.status)),
      onPress: () => { try { navigation.jumpTo('Tasks'); } catch { navigation.navigate('Tasks'); } },
    },
    {
      label: 'Overdue Tasks', value: overdueTasks, color: T.cRed, soft: T.cRedSoft, icon: 'alert-circle',
      pct: `${totalTasks > 0 ? Math.round((overdueTasks / totalTasks) * 100) : 0}% of total`,
      spark: sparkFor(t => {
        const due = t.end_date || t.due_date;
        return due && due < new Date().toISOString().slice(0, 10) && !['completed', 'deployed'].includes(t.status);
      }),
      onPress: () => { try { navigation.jumpTo('Tasks'); } catch { navigation.navigate('Tasks'); } },
    },
  ];
  ...
  const QUICK_ACTIONS = [
    { label: 'New Project', icon: 'folder-plus',  color: T.cBlue,   soft: T.cBlueSoft,   onPress: () => navigation.navigate('CreateProject') },
    { label: 'Upload Doc',  icon: 'upload',       color: T.cGreen,  soft: T.cGreenSoft,  onPress: () => navigation.navigate('Docs') },
    { label: 'New Task',    icon: 'check-square', color: T.cYellow, soft: T.cYellowSoft, onPress: () => navigation.navigate('CreateTask') },
    { label: 'Calendar',    icon: 'calendar',     color: T.cPurple, soft: T.cPurpleSoft, onPress: () => { try { navigation.jumpTo('Calendar'); } catch { navigation.navigate('Calendar'); } } },
    { label: 'Reports',     icon: 'bar-chart-2',  color: T.cBlue,   soft: T.cBlueSoft,   onPress: () => { try { navigation.jumpTo('Reports'); } catch { navigation.navigate('Reports'); } } },
    { label: 'Invite',      icon: 'user-plus',    color: T.cRed,    soft: T.cRedSoft,    onPress: () => navigation.navigate('InviteUser') },
  ];
      ...
      >
        {loading ? (
          ...
          <>
            ...
            >
              ...
              <View style={s.greetStats}>
                <View style={s.greetStat}>
                  <Text style={s.greetStatVal}>{tasks.filter(t => (t.status || '').toLowerCase() === 'pending' || (t.status || '').toLowerCase() === 'todo').length}</Text>
            ...
            {/* ── 3. Tasks by Status — Donut Chart ── */}
            <SectionHeader isDark={isDark} style={{ marginTop: 4 }}>Tasks by Status</SectionHeader>
                  ...
                  return (
                    <View style={{ width: SIZE, height: SIZE }}>
                      <Svg width={SIZE} height={SIZE}>
                        <Circle cx={cx} cy={cy} r={RADIUS} fill="none" stroke={isDark ? '#252530' : '#EBEBF0'} strokeWidth={STROKE} />
                        {segs.map((seg, i) => {
                          ...
                          offset    -= dash;
                          return (
                            <Circle key={i} cx={cx} cy={cy} r={RADIUS} fill="none" stroke={seg.color} strokeWidth={STROKE}
                              strokeDasharray={`${dash} ${gap}`} strokeDashoffset={co} strokeLinecap="butt" />
                          );
                        })}
                      </Svg>
                ...
                <View style={{ flex: 1, gap: 8 }}>
                  {[
                    ...
                    <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                      ...
                      <Text style={{ width: 78, fontSize: 11, color: isDark ? '#9AA3B2' : T.ink2, fontWeight: '500' }} numberOfLines={1}>{item.label}</Text>
                      ...
                      <Text style={{ fontSize: 11, fontWeight: '700', color: isDark ? '#fff' : T.ink, width: 26, textAlign: 'right' }}>{item.count}</Text>
                    </View>
                  ))}
                </View>
              </View>
            ...
            <Card isDark={isDark} style={{ marginBottom: 22 }} padding={12}>
              ...
              {tasksByTab(activeTaskTab).length === 0 ? (
                ...
                tasksByTab(activeTaskTab).map((task) => {
                ...
                projects.slice(0, 5).map((proj, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 13, fontWeight: '650', color: isDark ? '#fff' : T.ink }} numberOfLines={1}>{proj.name || 'Untitled'}</Text>
                        <Text style={{ fontSize: 11, color: isDark ? '#9AA3B2' : T.ink3, marginTop: 2 }}>
                          {taskCount} task{taskCount !== 1 ? 's' : ''}{memberCount > 0 ? ` · ${memberCount} member${memberCount !== 1 ? 's' : ''}` : ''}
                        </Text>
                      </View>
                      <View style={{ width: 80, marginRight: 10 }}>
                        <Progress value={progress} color={color} h={5} trackColor={isDark ? '#2A2A38' : T.hairlineSoft} />
                      </View>
                      <Text style={{ fontSize: 12, fontWeight: '650', color: isDark ? '#9AA3B2' : T.ink2, minWidth: 34, textAlign: 'right' }}>{progress}%</Text>
                    </TouchableOpacity>
                  );
                })
              )}
                ...
                recentDocs.map((doc, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 13, fontWeight: '600', color: isDark ? '#fff' : T.ink }} numberOfLines={1}>{doc.name || doc.file_name || 'Untitled'}</Text>
                        <Text style={{ fontSize: 11, color: isDark ? '#9AA3B2' : T.ink3, marginTop: 2 }} numberOfLines={1}>{path ? `Uploaded in ${path}` : 'Uploaded'}</Text>
                      </View>
            ...
            <View style={s.quickGrid}>
              {QUICK_ACTIONS.map((action, idx) => (
                <TouchableOpacity
                  key={idx}
                  style={[s.quickBtn, { backgroundColor: isDark ? '#1A1A20' : T.surface, borderColor: isDark ? '#252530' : T.hairline }]}
