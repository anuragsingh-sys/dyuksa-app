  ...
  const fetchWorkspaces = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setWorkspaces(Array.isArray(data) ? data : (data.results || data.workspaces || []));
      }
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      if (!contentType.includes('application/json')) {
        Alert.alert('Error', `Server error (${res.status}). Check the invite endpoint.`);
        return;
      }
      const data = await res.json();
      if (!res.ok) {
        Alert.alert('Error', data.detail || data.message || 'Failed to send invite.');
        return;
      }
