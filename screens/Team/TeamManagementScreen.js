...
export default function TeamManagementScreen() {
  ...
  const [loading, setLoading] = useState(!getCached('teams_management'));
  ...
  // ── Teams via SWR cache 
  const fetchTeamsFn = useCallback(async () => {
  ...
  const {
    ...
    setData: setTeamsCached,
  } = useCache('teams_management', fetchTeamsFn, { staleTime: 2 * 60 * 1000 });

  // Sync cached data to local state when it arrives
  useEffect(() => {
  ...
  useFocusEffect(useCallback(() => { fetchTeamsData(); }, [fetchTeamsData]));

  const fetchTeams = useCallback(async () => {
    invalidateCache('teams_management');
    const fresh = await fetchTeamsFn();
    setTeams(fresh);
    setTeamsCached(fresh);
    setLoading(false);
    setRefreshing(false);
  }, [fetchTeamsFn, fetchTeamsData]);
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      while (hasNext && page <= 10) {
        const data = await authApi.getUsers(page);
        all = [...all, ...(Array.isArray(data) ? data : [])];
        // authApi.getUsers returns flat array; check if there's more by seeing if we got a full page
        hasNext = Array.isArray(data) && data.length >= 20;
        page++;
      }
      all.sort((a, b) => {
 ...
 const handleAddUser = async () => {
    ...
    try {
      ...
      const d = e.data;
      const msg = d?.detail || d?.message || (d && Object.values(d)[0]?.[0]) || e.message || 'Failed to create user.';
