...
export default function RolesScreen() {
  ...
  const {
    ...
    setData: setUsers,
  } = useCache('roles_users', fetchUsersFn, { staleTime: 5 * 60 * 1000 });
  ...
  useFocusEffect(useCallback(() => { fetchUsersData(); }, [fetchUsersData]));

  const updateRole = async () => {
    if (!editUser || !newRole) return;
    setSaving(true);
    try {
      await authApi.updateRole(editUser.id, newRole);
      ...
      const msg = e.data?.detail || e.data?.message || e.message || 'Could not update role.';
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: bg }]} edges={['top', 'left', 'right']}>
      ...
      {/* User list */}
      {loading ? (
        <SkeletonList count={6} type="member" isDark={isDark} />
      ) : (
        <FlatList
          data={filtered}
          keyExtractor={u => String(u.id)}
          contentContainerStyle={{ padding: 12, paddingBottom: 40 }}
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={async () => { setRefreshing(true); invalidateCache('roles_users'); await fetchUsersData(true); setRefreshing(false); }} tintColor="#4ECDC4" />}
