...
export default function ChatScreen({ route }) {
  ...
  // ── Chat state from SWR cache hook ────────────────────────────────
  const {
    rooms,
  ...
  // Rooms fetching handled by useChat hook (SWR — no duplicate calls)
  // fetchUsersForModal still needed for New Group Chat modal
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: bg }]} edges={['top', 'left', 'right']}>
      ...
      <View style={{ flex: 1 }}>
      {loading ? (
        <SkeletonList count={6} type="chat" isDark={isDark} />
      ) : (
        <FlatList
          data={(() => {
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRoomsRefresh} tintColor="#3B72EE" />}
          ListEmptyComponent={
            <View style={styles.centerState}>
